...
if (msgclose) { 
 ...
 const invoiceUrl = `${rootUrl}/inc/ajax-invoice-check-ar.php`
...
$('#submit-button').on('click', function(e){
  //  e.preventDefault();
    ...
    if(fName.length < 3) {
        $('#fname-err').html('<p class="errormsg">الرجاء وضع الاسم و اللقب الصحيح</p>');
 ...
 //  if(!(mobile.length ===9)) { //|| !$.isNumeric(mobile)
      $('#mobile-err').html('<p class="errormsg">الرجاء ادخال رقم الهاتف الصحيح تنسيق الرقم: 0120000000 </p>');
  ...
  return regex.test(email);  */  
      $('#email-err').html('<p class="errormsg">الرجاء ادخال البريد الإلكتروني</p>');
  ...
  if(gender.length < 1 ) {
      $('#gender-err').html('<p class="errormsg">الرجاء تحديد الجنس</p>');
   ...
   if(city.length < 1 ) {
       $('#city-err').html('<p class="errormsg">الرجاء تحديد المنطقة</p>');
 ...
 if (!$('#customRadio1').prop('checked')) {
    $('#accept-err').html('<p class="errormsg text-danger">الرجاء قبول الأحكام و الشروط</p>');
    return false;
} else {
   ...
   // window.location.href = 'form.php'; // Redirect to form.php
    ...
    if (!$("#customRadio1").prop("checked")) {
        $('#accept-err').html('<p class="errormsg">الرجاء قبول الأحكام و الشروط</p>');
        return false;
    } else {
        // console.log(456);
...
$('#submit-receipt').on('click', function(){
    ...
    if(invNumber.length < 2) {
        $('#inv-number-err').html('<p class="errormsg">الرجاء ادخال رقم الايصال</p>');
